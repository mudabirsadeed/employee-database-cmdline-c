...
static void menu_delete_employee(void)
{
  ...
  for (i=0;i<total_employees;i++)
   {
	/*compares the input to al the names in array up to total_employees*/  
    j=strcmp(name, list[i].name);
    /*if a match is found, all of its fields are set to 0, resorted and the blank erased. L increments to show that a deletion has taken place*/
    if(j == 0)
     {
